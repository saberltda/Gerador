...
# =========================================================
# LÓGICA ORIGINAL: GENESIS V.50.2 (PURE SEARCH + STRICT Bairros.json)
# =========================================================

class GenesisConfig:
    VERSION = "GERADOR V.50.2 (PURE SEARCH / Bairros via JSON)"
...
def carregar_regras():
    """
    Carrega literalmente os blocos REGRAS_TECNICAS e REGRAS_TABELA do arquivo REGRAS.txt,
    sem resumir nem reescrever.
    """
    caminho = "REGRAS.txt"
    if not os.path.exists(caminho):
        raise RuntimeError(
            "Arquivo 'REGRAS.txt' não encontrado na pasta do aplicativo.\n\n"
            "▶ Coloque o arquivo REGRAS.txt na MESMA pasta do app e recarregue a página."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "Erro ao ler o arquivo 'REGRAS.txt'.\n\n"
            f"Detalhes técnicos: {e}\n\n"
            "▶ Verifique permissões e codificação do arquivo."
        )

    # Regex para pegar exatamente o bloco entre REGRAS_TECNICAS e REGRAS_TABELA
    # e o bloco entre REGRAS_TABELA e REGRAS_JSON-LD (ou fim do arquivo).
    m = re.search(
        r"REGRAS_TECNICAS\s*\n(.*?)\n\s*REGRAS_TABELA\s*\n(.*?)(?:\nREGRAS_JSON|$)",
        txt,
        re.DOTALL
    ...
    if not m:
        raise RuntimeError(
            "Arquivo 'REGRAS.txt' não está no formato esperado.\n\n"
            "▶ Certifique-se de que os blocos REGRAS_TECNICAS e REGRAS_TABELA "
            "estão presentes e nesta ordem."
        )

    regras_tecnicas = m.group(1).strip()
    regras_tabela = m.group(2).strip()
    return regras_tecnicas, regras_tabela
...
class BlogScanner:
    ...
    def mapear(self):
        ...
        try:
            ...
            # Falha silenciosa: se o feed cair, o gerador continua funcionando.
            pass
...
# =========================================================
# DATASET MESTRE (Bairros SEMPRE pelo JSON)
# =========================================================
...
class GenesisData:
    ...
    def _bairros(self):
        """
        Carrega SEMPRE do arquivo bairros.json.
        Se não conseguir ler ou se o conteúdo estiver inválido, lança RuntimeError.
        """
        caminho = "bairros.json"

        if not os.path.exists(caminho):
            raise RuntimeError(
                "Arquivo 'bairros.json' não encontrado na pasta do aplicativo.\n\n"
                "▶ Coloque o arquivo de bairros na MESMA pasta do app e recarregue a página."
            )
        ...
        try:
            ...
            raise RuntimeError(
                "Erro ao ler o arquivo 'bairros.json'.\n\n"
                f"Detalhes técnicos: {e}\n\n"
        ...
        if not isinstance(raw, list) or not raw:
            raise RuntimeError(
                "O arquivo 'bairros.json' foi carregado, mas não contém uma lista de bairros válida.\n\n"
                "▶ O conteúdo deve ser uma lista JSON de objetos com chaves 'nome' e 'zona'."
            )
        ...
        def _map_zona(zona_texto: str):
            ...
            if "industrial" in z or "empresarial" in z:
                return "industrial"
            if "condomínio residencial fechado" in z:
                return "residencial_fechado"
            if "condomínio de chácaras" in z:
        ...
        for b in raw:
            # tolerante a registros quebrados, mas ignora os inválidos
            nome = b.get("nome")
            zona = b.get("zona", "")
            if not nome or not isinstance(nome, str):
                continue
            b2 = dict(b)
            b2["slug"] = slugify(nome)
            b2["zona_normalizada"] = _map_zona(zona)
            bairros_enriquecidos.append(b2)

        if not bairros_enriquecidos:
            raise RuntimeError(
                "Nenhum bairro válido foi encontrado em 'bairros.json'.\n\n"
                "▶ Verifique se os registros possuem pelo menos 'nome' e 'zona'."
            )
...
class GenesisEngine:
    def __init__(self):
        ...
        self.data = GenesisData()  # aqui já tenta ler bairros.json
...
# =========================================================
# PROMPT BUILDER (usa REGRAS.txt literal)
# =========================================================
...
class PromptBuilder:
    ...
    def get_strict_table_rules(self):
        """
        Retorna literalmente os blocos REGRAS_TECNICAS e REGRAS_TABELA do REGRAS.txt.
        """
        regras_tecnicas, regras_tabela = carregar_regras()
        return f"{regras_tecnicas}\n\n{regras_tabela}"
...
""" % (data_pub, data_mod)
        ...
        **ÂNCORAS LOCAIS (SEARCH MODE):** - A IA DEVE EXECUTAR UMA PESQUISA WEB (Search/Browse) para identificar o entorno do local: {loc}.
        ...
        regras_html_tabela = self.get_strict_table_rules()

        return f"""
## GENESIS MAGNETO V.50.2 — PURE SEARCH
...
## 3. REGRAS TÉCNICAS E VISUAIS (REGRAS.txt)
{regras_html_tabela}
...
# =========================================================
# UI STREAMLIT (VISUAL V8 | LÓGICA V50.2)
# =========================================================

def main():
    # Cores da UI do Streamlit (Copiado do V8)
    THEME = {
    ...
    st.set_page_config(page_title="Genesis Agency v11", page_icon="💎", layout="wide")
    ...
    with c1:
        st.title("💎 GENESIS AGENCY V11.0")
        st.markdown(f"**AI Content Director para Imobiliária Saber (Engine: {GenesisConfig.VERSION})**")
    with c2:
        st.image(
            "https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEixiE1KghKkH0E-I53yyi5zoT7eRX0lxCGLpcWLGAmEE5st8OfHfuzbxfiygwCWRqAdSfpmjAhM8-SogHDU_1gXCX6IHrjW1BaUc87un1lF1o6y2Et7eV0m3gJgvfJs3HsAGyAcPYk8Tl_65rlQmgAp5orRZqtLDvixbCUwscTT8ZJO-7zckc36rNkWHz4/s1600/1000318124.png",  # noqa: E501
            width=100
        )
    ...
    with col_main:
        ...
        st.write("O sistema irá selecionar a melhor oportunidade baseada no inventário e personas (V50.2 Logic).")
        generate_btn = st.button("CRIAR PAUTA ESTRATÉGICA ✨")

    if generate_btn:
        # EXECUÇÃO DA LÓGICA
        with st.spinner("Escaneando blog, carregando arquivos e aplicando lógica híbrida V50.2..."):
            try:
                eng = GenesisEngine()
                bld = PromptBuilder()
                ...
                nome_arquivo = f"{d_pub.split('T')[0]}_V50_2_{p_name}_{ativo_name}.txt"

            except RuntimeError as e:
                st.error(
                    "❌ Não foi possível carregar um dos arquivos obrigatórios "
                    "(bairros.json ou REGRAS.txt).\n\n"
                    f"{e}\n\n"
                    "O programa foi interrompido para evitar resultados incoerentes."
                )
                st.stop()
            except Exception as e:
                st.error("❌ Erro inesperado ao gerar a pauta.")
                st.exception(e)
                st.stop()

        # Exibição do "Raciocínio da Agência"
        bairro_display = dados['bairro']['nome'] if dados['bairro'] else "Indaiatuba (Panorama Geral)"
        zona_display = dados['bairro']['zona'] if dados['bairro'] else "Macro-zoneamento"
        ...
        st.markdown("### 📋 Prompt de Engenharia Reversa (V50.2)")

...
class GenesisConfig:
    VERSION = "GENESIS V.50.1 (PURE SEARCH)" # Mantido nome técnico, visual atualizado

    # Design System (Herdado do V9 para Visual)
    THEME = {
        "primary": "#003366", 
        "accent": "#D4AF37",  
        "bg": "#f4f6f9"
    ...
    # URLs Técnicas (Mantidas do V50.1)
    COLOR_PRIMARY = "#003366"   
    BLOG_URL = "https://blog.saber.imb.br"
    FUSO_PADRAO = "-03:00"

    # REGRAS DE SEGURANÇA (Mantidas do V50.1)
    STRICT_GUIDELINES = [
    ...
    # =====================================================
    # 1. MATRIZ DE PERSONAS (ARQUÉTIPOS) - MANTIDA V50.1
    # =====================================================
    PERSONAS = {
        "EXODUS_SP_FAMILY": {
            "cluster_ref": "FAMILY",
...
# =========================================================
# PLANO DIRETOR (LÓGICA - MANTIDO V50.1)
# =========================================================
...
# =========================================================
# GENESIS ENGINE V50.1 (CORE - MANTIDO)
# =========================================================
...
# =========================================================
# PROMPT BUILDER V50.1 (FIXED - MANTIDO)
# =========================================================
...
# =========================================================
# INTERFACE STREAMLIT (VISUAL V9 + LÓGICA V50.1)
# =========================================================

def main():
    st.set_page_config(page_title="Genesis Agency v.50.1", page_icon="💎", layout="wide")

    # CSS INJETADO (Do app_gerador_9)
    st.markdown(f"""
    <style>
        .stApp {{ background-color: {GenesisConfig.THEME['bg']}; }}
        .big-card {{ 
            background: white; padding: 20px; border-radius: 10px; 
            border-left: 6px solid {GenesisConfig.THEME['primary']};
            box-shadow: 0 4px 6px rgba(0,0,0,0.1); margin-bottom: 20px;
        }}
        .stat-value {{ font-size: 24px; font-weight: bold; color: {GenesisConfig.THEME['primary']}; }}
        .stat-label {{ font-size: 14px; color: #666; text-transform: uppercase; letter-spacing: 1px; }}
        .highlight {{ color: {GenesisConfig.THEME['accent']}; font-weight: bold; }}
        div.stButton > button {{
            background: linear-gradient(45deg, {GenesisConfig.THEME['primary']}, #004080);
    ...
    with c1:
        st.title("💎 GENESIS AGENCY V.50.1")
        st.markdown(f"**Pure Search Mode & Compliance Director**")
    with c2:
        st.image("[https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEixiE1KghKkH0E-I53yyi5zoT7eRX0lxCGLpcWLGAmEE5st8OfHfuzbxfiygwCWRqAdSfpmjAhM8-SogHDU_1gXCX6IHrjW1BaUc87un1lF1o6y2Et7eV0m3gJgvfJs3HsAGyAcPYk8Tl_65rlQmgAp5orRZqtLDvixbCUwscTT8ZJO-7zckc36rNkWHz4/s1600/1000318124.png](https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEixiE1KghKkH0E-I53yyi5zoT7eRX0lxCGLpcWLGAmEE5st8OfHfuzbxfiygwCWRqAdSfpmjAhM8-SogHDU_1gXCX6IHrjW1BaUc87un1lF1o6y2Et7eV0m3gJgvfJs3HsAGyAcPYk8Tl_65rlQmgAp5orRZqtLDvixbCUwscTT8ZJO-7zckc36rNkWHz4/s1600/1000318124.png)", width=100)

    # SIDEBAR (Layout V9 + Inputs V50.1)
    with st.sidebar:
        st.header("⚙️ Configuração")
        data_escolhida = st.date_input("Data de Publicação", datetime.date.today())
        
        st.markdown("---")
        st.success("PROTOCOLOS V50.1 ATIVOS")
        st.caption("✅ Pure Search Logic")
        st.caption("✅ Anti-Hallucination")
        st.caption("✅ Strict Table CSS")
    ...
    with col_main:
        st.markdown("### Gerar Pauta")
        btn_gerar = st.button("CRIAR PAUTA COMPLIANT ✨")

    if btn_gerar:
        with st.spinner("🤖 Escaneando blog e aplicando lógica híbrida..."):
            # Instancia as classes (Lógica V50.1)
            eng = GenesisEngine()
            bld = PromptBuilder()
        ...
        # VISUALIZAÇÃO EM CARDS (Estilo V9)
        with col_view:
            bairro_nome = dados['bairro']['nome'] if dados['bairro'] else 'Cidade (Geral)'
            st.markdown(f"""
            <div class="big-card">
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    ...
                    <div>
                        <div class="stat-label">Bairro/Foco</div>
                        <div class="stat-value">{bairro_nome}</div>
                    </div>
                    <div>
                        <div class="stat-label">Cluster</div>
                        <div class="stat-value">{dados['cluster_tecnico']}</div>
                    </div>
                    <div>
                        <div class="stat-label">Ativo</div>
                        <div class="stat-value" style="font-size: 18px;">{dados['ativo_definido']}</div>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            
            st.info(f"**Formato:** {dados['formato']} | **Gatilho:** {dados['gatilho']}")

        st.markdown("### 📋 Prompt Gerado (Pronto para Copiar)")
        st.text_area("Copie o texto abaixo:", value=prompt_final, height=500)
        
        st.download_button("💾 BAIXAR ARQUIVO TXT", prompt_final, nome_arquivo)

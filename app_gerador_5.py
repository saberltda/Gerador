...
# =========================================================
# CONFIGURAÇÃO: GENESIS V.66 (SECURITY PROTOCOLS)
# =========================================================

class GenesisConfig:
    VERSION = "GENESIS V.66 (SECURITY PROTOCOLS)"
    ...
    # ==============================================================================
    # ⛔ ZONA DE SEGURANÇA MÁXIMA (IMPORTADO DE REGRAS.TXT)
    # ==============================================================================
...
"""
    ...
    # REGRAS DE SEGURANÇA & ANTI-ALUCINAÇÃO (LÓGICA)
    STRICT_GUIDELINES = [
...
# =========================================================
# PROMPT BUILDER (AGORA BLINDADO COM AS REGRAS)
# =========================================================
...
class PromptBuilder:
    def build(self, d, data_pub, data_mod):
        ...
        # MONTAGEM DO PROMPT COM INJEÇÃO DAS LEIS DA FÍSICA (REGRAS_TECNICAS e REGRAS_TABELA)
        return f"""
...
3. **TABELA DE DADOS (CSS PURO):**
   Gere uma tabela comparativa ou de dados.
   {GenesisConfig.REGRAS_TABELA}

{GenesisConfig.REGRAS_TECNICAS}
...
def main():
    st.set_page_config(page_title="Genesis V.66", page_icon="🛡️", layout="wide")
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.info("**Logic Shield:** Ativo\n\n**Safety Zones:** Ativo\n\nImpede combinações bizarras e formatação quebrada.")

    # HEADER
    st.markdown("## 🏙️ Gerador de Pautas Imobiliárias")
    st.markdown(f"**Imobiliária Saber** | Intelligence Engine V.66")
    ...
    with tab_gerador:
        ...
        with col_info:
            ...
            O sistema validará a lógica e aplicará as regras de segurança HTML.
            </div>
            """, unsafe_allow_html=True)
        ...
        if btn_gerar:
            ...
            with st.status("🔄 Processando Inteligência...", expanded=True) as status:
                ...
                d_pub = data_escolhida.strftime("%d de %B de %Y") # Formato mais amigável para o prompt
            ...
            with col_txt:
                st.subheader("📋 Prompt Final (Regras Injetadas)")
            ...
            with col_down:
                ...
                st.info("Otimizado para Claude 3.5 Sonnet / GPT-4o.")

    # --- ABA 2: MANUAL MASTERCLASS ---
    with tab_manual:
        st.markdown("""
        # 📘 Manual de Operações: Genesis V.66
        
        **ATENÇÃO: ATUALIZAÇÃO DE SEGURANÇA (V.66)**
        Esta versão agora incorpora rigidamente as "Leis da Física" do template do Blogger. O prompt gerado contem instruções de CSS inline e ordem de tags que **não devem ser alteradas** pela IA.
        ...
        4. O sistema filtra imóveis incompatíveis (ex: Galpão em Bairro Residencial).

        ### 🛡️ Protocolos de Segurança (Novo)
        O prompt final agora carrega blocos de texto que instruem a IA a:
        * **NÃO** usar tags `<html>`, `<body>`.
        * **USAR** CSS específico nas tabelas para não quebrar no celular.
        * **ORDENAR** a saída num checklist específico (Log > HTML > Título...).
        ...
        | Problema | Solução |
        | :--- | :--- |
        | **Tabela ficou feia no celular** | Verifique se a IA respeitou o `REGRAS_TABELA` injetado no prompt. |
        | **O post quebrou o site** | A IA provavelmente inseriu `<!DOCTYPE html>`. O novo prompt proíbe isso explicitamente. |
        """)

...
class BlogScanner:
    ...
    def mapear(self):
        self.bairros_publicados = set()
        self.todos_titulos = []
        print(f"📡 [SCANNER V50.1] Verificando sitemap e histórico...")
        try:
            ...
            with urllib.request.urlopen(self.feed_url, context=ctx, timeout=20) as response:
                if response.status == 200:
                    ...
                    if "feed" in data and "entry" in data["feed"]:
                        ...
                        print(f"✅ [SYNC OK] {len(self.todos_titulos)} artigos indexados.")
                    else:
                        print("⚠️ [AVISO] Feed vazio.")
                else:
                    raise ConnectionError(f"Status: {response.status}")
        except Exception as e:
            print(f"⚠️ [AVISO] Falha na conexão com o blog: {e}. Prosseguindo sem histórico.")
...
class GenesisData:
    def __init__(self):
        ...
        # REMOVIDO: Nenhuma entidade local hardcoded.
...
class GenesisEngine:
    ...
    def run(self):
        ...
        print("\n🎰 [GENESIS V50.1] Executando Lógica Híbrida...")
        ...
        print(f"🎯 Alvo: {persona_data['nome']} (Cluster: {cluster_ref})")
...
class PromptBuilder:
    ...
    def get_strict_table_rules(self):
        # FIX V50.1: REMOVIDOS TODOS OS EXEMPLOS HARDCODED
        # A instrução agora obriga a busca.
...
def input_datas():
    print("\n" + "="*50)
    print(f"=== 🚀 {GenesisConfig.VERSION} ===")
    try: data_str = input("➡ Data (DD/MM/AAAA) [Enter=Hoje]: ").strip()
    except: return None, None
    if data_str.lower() in ["sair", "exit"]: return None, None
    
    hoje = datetime.datetime.now()
    hoje_iso = hoje.strftime(f"%Y-%m-%dT%H:%M:%S{GenesisConfig.FUSO_PADRAO}")
    if not data_str: return hoje_iso, hoje_iso
    
    try:
        dt = datetime.datetime.strptime(data_str, "%d/%m/%Y")
        dt_iso = dt.strftime(f"%Y-%m-%dT00:00:00{GenesisConfig.FUSO_PADRAO}")
        return dt_iso, hoje_iso
    except: return hoje_iso, hoje_iso
...
if __name__ == "__main__":
    ...
    while True:
        d_pub, d_mod = input_datas()
        if d_pub is None: break

        dados = eng.run()
        prompt = bld.build(dados, d_pub, d_mod)
        ...
        print(f"\n✅ PROMPT V50.1 GERADO: {nome_arquivo}")
        print(f"🎯 Arquétipo: {dados['persona']['nome']}")
        print(f"🔍 Mode: Pure Search (Sem Âncoras Fixas)")
        print("-" * 50)
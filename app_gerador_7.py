...
class ImmutableRules:
    ...
    # [cite_start]REGRA 02: Estrutura Técnica HTML (Blogger)[cite: 1, 2, 3, 4].
    REGRAS_TECNICAS = """
...
"""
    ...
    # [cite_start]REGRA 03: Design de Tabelas Anti-Quebra[cite: 5, 6, 7].
    REGRAS_TABELA = """
...
class GenesisConfig:
    VERSION = "APP_GERADOR_7 (IMMUTABLE CORE)"
    COLOR_PRIMARY = "#003366"   
    COLOR_ACCENT = "#FF4B4B"
    BLOG_URL = "https://blog.saber.imb.br"
    FUSO_PADRAO = "-03:00"

    # Vocabulário Proibido (Anti-Clichê)
    FORBIDDEN_WORDS = [
...
class PlanoDiretor:
    def validar_e_corrigir(self, cluster_key, ativo_sugerido, bairro_obj):
        zona_norm = bairro_obj.get("zona_normalizada", "indefinido")
...
class BlogScanner:
    def __init__(self, blog_url=GenesisConfig.BLOG_URL):
        # Implementação da REGRA 01 das Regras Imutáveis
    ...
    def mapear(self):
        ...
        try:
            ...
            # Fallback silencioso para não travar o app se estiver offline
            status_msg = f"Modo Offline (Erro: {str(e)})"
        
        return status_msg

    def verificar_canibalizacao(self, nome_bairro, topico):
        """Retorna True se houver risco alto de repetição"""
        ...
        return conflitos[:5] # Retorna os 5 mais recentes conflitos
...
class GenesisData:
    def get_bairros(self):
        return [
            ...
            {"nome": "Parque Ecológico", "zona": "Mista/Vertical", "zona_normalizada": "zona_vertical"},
            ...
            {"nome": "Distrito Industrial Europark", "zona": "Zona Industrial", "zona_normalizada": "industrial"},
...
class GenesisEngine:
    ...
    def run(self):
        ...
        # 2. Filtrar Bairros
        todos_bairros = self.data.get_bairros()
        candidatos = [b for b in todos_bairros if b['zona_normalizada'] in persona['preferred_zones']]
        if not candidatos: candidatos = todos_bairros 
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption("Immutable Core Edition")
        st.markdown("---")
        data_escolhida = st.date_input("📅 Data de Publicação", datetime.date.today())
        
        st.success("**REGRAS IMUTÁVEIS:**")
        st.markdown("- ✅ HTML Seguro (Blogger)")
        st.markdown("- ✅ Tabelas Anti-Quebra")
        st.markdown(f"- ✅ Scan de {ImmutableRules.MAX_RESULTS_SCAN} Posts")
    ...
    with col_info:
        st.info("O sistema consulta o histórico do blog para evitar repetições e aplica o Logic Shield para evitar erros de zoneamento.")
    ...
    if btn_gerar:
        ...
        with st.status("🔄 Processando Inteligência...", expanded=True) as status:
            ...
            st.write(f"📡 Status do Scan: {dados['scan_status']}")
            if dados['conflitos']:
                st.warning(f"⚠️ {len(dados['conflitos'])} Artigos conflitantes encontrados (Anti-Canibalização Ativado).")
